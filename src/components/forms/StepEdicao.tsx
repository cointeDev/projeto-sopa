...
export default function StepEdicao() {
	const {
		passo,
		setPassoAtual,
		validarPassoAtual,
				...
				<div>
					...
					<label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-slate-200 rounded-[2.5rem] cursor-pointer bg-[#F8FAFC] hover:bg-white transition-all group shadow-inner">
						<div className="flex flex-col items-center justify-center pt-5 pb-6">
							...
							<p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">
								{formData.materialEdicao
									? formData.materialEdicao.name
									: "Clique para anexar o material de edição"}
							</p>
						</div>
						<input
							accept=".pdf,.doc,.docx,.txt"
							...
							onChange={(event_) => {
								const file = event_.target.files?.[0];
								if (file) updateField("materialEdicao", file);
							}}
						/>
					</label>
				</div>
