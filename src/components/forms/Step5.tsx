...
import {
	ACESSIBILIDADE_LABELS /*type Acessibilidade*/,
	...
	async function handleConfirmar() {
		try {
			const payload = {
				...
				data: formData.data,
				hora: formData.hora,
				TipoProducao: formData.TipoProducao,
				FormatoProducao: formData.FormatoProducao,
					...
					<div className="space-y-2 text-sm">
						<p className="text-slate-400 font-medium">
							...
							<span className="text-[#334155] font-bold">
								{formData.nomeProjeto || "Não informado"}
							</span>
						</p>
						<p className="text-slate-400 font-medium">
							Título:{" "}
					...
					<div className="bg-[#F8FAFC] p-6 rounded-3xl border border-slate-100">
						...
						{formData.observacoes && (
							<p className="text-sm mt-4 pt-4 border-t border-slate-200/50">
								<span className="text-[#334155] font-black uppercase text-[9px] tracking-widest">
									Observações:
								</span>{" "}
